<template>
    <div class="container">
        <div class="config_contents">
            <div class="input_contents">
                <div class="planet_select">
                    ...
                    />
                    <p>
                        ※惑星を選択するとその惑星に応じた第一宇宙速度が入力されます（誤差により＋1）
                    </p>
                </div>
                <div class="textbox_input">
                    <div class="planet_info">
                        ...
                        <CustomInput
                            ...
                            :name="'time_speed'"
                            :label="'倍速時間(s)'"
                            v-model="timeSpeedRef"
                            :disableFlag="inputDisableFlagRef"
                            :placeholder="'正の整数で入力'"
                            @input="timeSpeedRef = $event"
                        />
                    </div>
                </div>
            </div>
        ...
        />
        <ConfirmationModal
            v-show="isConfirmationRef"
            :message="confirmationMsgRef"
            @ok="confirmationModalOk"
            @cancel="confirmationModalClose"
        />
    </div>
</template>
...
const calcStart = async () => {
    ...
    gravityCoefficientRef.value =
        parseFloat(Const.G) * parseFloat(massRef.value);
